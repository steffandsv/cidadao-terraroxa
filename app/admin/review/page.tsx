                        ...
                        <div key={action.id} className="bg-white rounded-xl shadow-sm border overflow-hidden">
                            {action.evidenceUrl && (
                                <div className="h-48 bg-gray-200 overflow-hidden relative">
                                    <img src={action.evidenceUrl} alt="Prova" className="w-full h-full object-cover" />
                                </div>
                            )}
                            <div className="p-4">
                                <div className="flex justify-between items-start mb-2">
                                    <span className="font-bold text-gray-800">{action.rule.slug}</span>
                                    <span className="text-xs text-gray-500">{new Date(action.createdAt).toLocaleDateString()}</span>
                                </div>
                                <p className="text-sm text-gray-600 mb-4">
                                    Usuário: {action.user.name || action.user.phone} <br/>
                                    Local: {action.asset ? action.asset.description : 'Geral'}
                                </p>

                                <div className="flex gap-2 mt-4">
                                    <form action={reviewAction.bind(null, action.id, 'APPROVED')} className="flex-1">
                                        <button className="w-full flex items-center justify-center gap-2 bg-emerald-100 text-emerald-700 py-2 rounded-lg hover:bg-emerald-200 transition">
                                            <Check size={18} /> Aprovar
                                        </button>
                                    </form>
                                    <form action={reviewAction.bind(null, action.id, 'REJECTED')} className="flex-1">
                                         <button className="w-full flex items-center justify-center gap-2 bg-red-100 text-red-700 py-2 rounded-lg hover:bg-red-200 transition">
                                            <X size={18} /> Rejeitar
                                        </button>
                                    </form>
                                </div>
